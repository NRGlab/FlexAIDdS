...
TEST_F(StatMechEngineTest, EntropyIncreasesWithSpread) {
    // At fixed T, narrower energy spread → more equal Boltzmann weights → more entropy.
    // Wide energy spread → lowest-energy state dominates → less entropy.
    StatMechEngine narrow(TEMPERATURE);
    StatMechEngine broad(TEMPERATURE);

    for (int i = 0; i < 5; ++i) {
        narrow.add_sample(-10.0 - 0.01 * i);  // very tight → nearly equal weights
        broad.add_sample(-10.0 - 5.0 * i);    // wide spread → one state dominates
    }

    auto th_narrow = narrow.compute();
    auto th_broad = broad.compute();

    // Narrow spread gives MORE entropy (more uniform distribution)
...
TEST_F(StatMechEngineTest, HighTemperatureFlattensWeights) {
    ...
    // Need kT >> energy range for near-uniform weights.
    // With energies spanning 30 kcal/mol, need T ≫ 30/kB ≈ 15000K.
    StatMechEngine hot(500000.0);  // very high T → kT ≈ 993 kcal/mol
